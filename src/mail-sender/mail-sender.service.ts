...
export class MailSenderService {
  ...
  private accessToken: () => Promise<any>;
  ...
  async sendVerifyEmail(emailTo: string) {
    try {
      const mailOptions = {
        from: 'Hello <daniel@dandelionlabs.io>',
        to: emailTo,
  ...
  async sendVerifyEmailMail(name: string, email: string, token: string): Promise<boolean> {
    ...
    const mail = confirmMail
      ...
      .replace(new RegExp('--TermsOfServiceLink--', 'g'), config.project.termsOfServiceUrl);
